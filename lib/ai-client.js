...
async function chat(opts) {
  ...
  const provider = forceProvider || providerName();
  if (!provider) return { ok: false, error: 'no_provider' };

  switch (provider) {
    case 'gemini': return geminiChat({ system, messages: normalized, maxTokens, temperature, jsonMode });
    case 'grok':   return grokChat({ system, messages: normalized, maxTokens, temperature, jsonMode });
    case 'openai': return openaiChat({ system, messages: normalized, maxTokens, temperature, jsonMode });
    case 'anthropic': return anthropicChat({ system, messages: normalized, maxTokens, temperature });
