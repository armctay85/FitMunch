...
window.fitMunchAPI = {
  ...
  saveProfile: async function(userId, profile) {
    return await this.apiCall('/api/user/profile', 'POST', { userId, profile });
  },

  getProfile: async function(userId) {
    const result = await this.apiCall(`/api/user/profile/${userId}`, 'GET');
    return result.profile;
  },

  // Meal Logging API calls
  logMeal: async function(userId, date, mealType, foodItems, calories, macros) {
    return await this.apiCall('/api/meals/log', 'POST', {
      userId,
      date,
      mealType,
      foodItems,
  ...
  getDailyMeals: async function(userId, date) {
    const result = await this.apiCall(`/api/meals/daily/${userId}/${date}`, 'GET');
    return result.data;
  },

  // Workout Logging API calls
  logWorkout: async function(userId, date, workoutType, duration, caloriesBurned, exercises) {
    return await this.apiCall('/api/workouts/log', 'POST', {
      userId,
      date,
      workoutType,
      duration,
  ...
  getWorkoutHistory: async function(userId, limit = 30) {
    const result = await this.apiCall(`/api/workouts/history/${userId}?limit=${limit}`, 'GET');
    return result.workouts;
  },

  // Progress Tracking API calls
  logProgress: async function(userId, date, weight, bodyFat, measurements, notes) {
    return await this.apiCall('/api/progress/log', 'POST', {
      userId,
      date,
      weight,
      bodyFat,
  ...
  getProgressHistory: async function(userId, limit = 30) {
    const result = await this.apiCall(`/api/progress/history/${userId}?limit=${limit}`, 'GET');
    return result.progress;
  },

  // Nutrition data functions (mock - for external API searches)
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = window.fitMunchAPI;
}