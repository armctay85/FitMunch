...
const { pgTable, serial, text, integer, timestamp, boolean, jsonb, real, varchar, uuid } = require('drizzle-orm/pg-core');
...
const progressLogs = pgTable('progress_logs', {
  id: serial('id').primaryKey(),
  userId: uuid('user_id').references(() => users.id, { onDelete: 'cascade' }).notNull(),
  date: timestamp('date').defaultNow().notNull(),
  weight: real('weight'),
...
const userProfilesRelations = relations(userProfiles, ({ one }) => ({
  user: one(users, {
...
const mealLogsRelations = relations(mealLogs, ({ one }) => ({
  user: one(users, {
...
const progressLogsRelations = relations(progressLogs, ({ one }) => ({
  user: one(users, {
